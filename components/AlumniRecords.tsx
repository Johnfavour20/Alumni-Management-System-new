...
import type { Alumni, ViewMode, SortBy, SortOrder, User } from '../types';
import { 
...
const AlumniRecords: React.FC<Props> = ({ alumni, currentUser, animateCards, openModal, confirmDelete, onStartConversation }) => {
  ...
  const filteredAlumni = useMemo(() => {
    let alumniToShow = isStudentView ? alumni.filter(a => a.openToMentoring) : alumni;
    
    return alumniToShow.filter(person => {
      const matchesSearch = 
      ...
      if (sortBy === 'salary') {
        aVal = Number(a.salary);
        bVal = Number(b.salary);
      }
  ...
  }, [alumni, searchTerm, filters, sortBy, sortOrder, isStudentView]);
  ...
  return (
    <div className="p-4 lg:p-8">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 space-y-4 sm:space-y-0">
        <div>
          <h2 className="text-3xl lg:text-4xl font-bold bg-gradient-to-r from-green-500 to-green-700 bg-clip-text text-transparent mb-2">
            {isStudentView ? 'Find a Mentor' : 'Alumni Records'}
          </h2>
          <p className="text-lg text-gray-700 dark:text-gray-300">
            {isStudentView ? 'Connect with experienced alumni for guidance' : 'Manage and view all alumni information'}
          </p>
        </div>
        {!isStudentView && (
      ...
      <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-lg mb-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 items-center">
          ...
          <div className="lg:col-span-6 grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <select value={filters.isActive} onChange={(e) => setFilters({...filters, isActive: e.target.value})} className="px-3 py-3 rounded-lg border border-gray-300 dark:border-gray-600 focus:border-green-500 focus:outline-none dark:bg-gray-700 dark:text-white transition-colors" disabled={isStudentView}><option value="">All Status</option><option value="true">Active</option><option value="false">Inactive</option></select>
          </div>
      ...
      <div className="flex justify-between items-center mb-6">
        <p className="text-gray-700 dark:text-gray-300">Showing {filteredAlumni.length} of {isStudentView ? alumni.filter(a => a.openToMentoring).length : alumni.length} alumni</p>
        <select value={sortBy} onChange={(e) => setSortBy(e.target.value as SortBy)} className="px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-600 focus:border-green-500 focus:outline-none dark:bg-gray-700 dark:text-white transition-colors text-sm"><option value="firstName">Sort by Name</option><option value="graduationYear">Sort by Year</option><option value="company">Sort by Company</option>{!isStudentView && <option value="salary">Sort by Salary</option>}</select>
      </div>

      {viewMode === 'grid' ? (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">{filteredAlumni.map((person, index) => (<div key={person.id} className={`bg-white dark:bg-gray-800 rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-500 transform hover:scale-105 ${animateCards ? 'animate-fade-in' : 'opacity-0'}`} style={{ animationDelay: `${index * 50}ms` }}><div className="p-6"><div className="flex items-center space-x-4 mb-4"><div className="w-16 h-16 bg-gradient-to-r from-green-500 to-green-700 rounded-full flex items-center justify-center text-white text-xl font-bold">{person.firstName.charAt(0)}{person.lastName.charAt(0)}</div><div className="flex-1"><h3 className="text-lg font-bold text-gray-900 dark:text-white">{person.firstName} {person.lastName}</h3><p className="text-sm text-gray-700 dark:text-gray-300">{person.currentPosition}</p></div><div className={`px-2 py-1 rounded-full text-xs font-medium ${person.isActive ? 'bg-green-100 text-green-800 dark:bg-green-800 dark:text-green-100' : 'bg-gray-100 text-gray-800 dark:bg-gray-600 dark:text-gray-300'}`}>{person.isActive ? 'Active' : 'Inactive'}</div></div><div className="space-y-3 mb-4"><div className="flex items-center space-x-2 text-sm"><Building2 className="h-4 w-4 text-green-500" /><span className="text-gray-700 dark:text-gray-300 truncate">{person.company}</span></div><div className="flex items-center space-x-2 text-sm"><GraduationCap className="h-4 w-4 text-green-500" /><span className="text-gray-700 dark:text-gray-300">{person.degree} {person.graduationYear}</span></div><div className="flex items-center space-x-2 text-sm"><MapPin className="h-4 w-4 text-gray-500" /><span className="text-gray-700 dark:text-gray-300 truncate">{person.location}</span></div></div><div className="flex items-center justify-between pt-4 border-t border-gray-200 dark:border-gray-600"><div className="flex space-x-2"><button onClick={() => openModal('view', person)} className="p-2 bg-green-100 dark:bg-green-800 text-green-600 dark:text-green-300 rounded-lg hover:bg-green-200 dark:hover:bg-green-700 transition-colors" title="View Details"><Eye className="h-4 w-4" /></button>{!isStudentView && <button onClick={() => openModal('edit', person)} className="p-2 bg-green-100 dark:bg-green-800 text-green-600 dark:text-green-300 rounded-lg hover:bg-green-200 dark:hover:bg-green-700 transition-colors" title="Edit"><Edit className="h-4 w-4" /></button>}<button onClick={() => onStartConversation(person.id)} className="p-2 bg-teal-100 dark:bg-teal-800 text-teal-600 dark:text-teal-300 rounded-lg hover:bg-teal-200 dark:hover:bg-teal-700 transition-colors" title={isStudentView ? 'Request to Connect' : 'Message'}><Mail className="h-4 w-4" /></button>{!isStudentView && <button onClick={() => confirmDelete(person.id)} className="p-2 bg-red-100 dark:bg-red-800 text-red-600 dark:text-red-300 rounded-lg hover:bg-red-200 dark:hover:bg-red-700 transition-colors" title="Delete"><Trash2 className="h-4 w-4" /></button>}</div><div className="text-sm text-gray-600 dark:text-gray-400">{person.lastLogin}</div></div></div></div>))}</div>
      ) : (
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg overflow-hidden"><div className="overflow-x-auto"><table className="w-full"><thead className="bg-gray-50 dark:bg-gray-700"><tr><th className="px-6 py-4 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Name</th><th className="px-6 py-4 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider hidden sm:table-cell">Email</th><th className="px-6 py-4 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider hidden md:table-cell">Company</th><th className="px-6 py-4 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider hidden lg:table-cell">Position</th><th className="px-6 py-4 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Status</th><th className="px-6 py-4 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Actions</th></tr></thead><tbody className="divide-y divide-gray-200 dark:divide-gray-600">{filteredAlumni.map((person) => (<tr key={person.id} className="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors"><td className="px-6 py-4"><div className="flex items-center space-x-3"><div className="w-10 h-10 bg-gradient-to-r from-green-500 to-green-700 rounded-full flex items-center justify-center text-white font-semibold text-sm">{person.firstName.charAt(0)}{person.lastName.charAt(0)}</div><div><div className="font-medium text-gray-900 dark:text-white">{person.firstName} {person.lastName}</div><div className="text-sm text-gray-600 dark:text-gray-300">{person.degree} {person.graduationYear}</div></div></div></td><td className="px-6 py-4 text-sm text-gray-900 dark:text-white hidden sm:table-cell truncate">{person.email}</td><td className="px-6 py-4 text-sm text-gray-900 dark:text-white hidden md:table-cell truncate">{person.company}</td><td className="px-6 py-4 text-sm text-gray-900 dark:text-white hidden lg:table-cell truncate">{person.currentPosition}</td><td className="px-6 py-4"><span className={`px-2 py-1 rounded-full text-xs font-medium ${person.isActive ? 'bg-green-100 text-green-800 dark:bg-green-800 dark:text-green-100' : 'bg-gray-100 text-gray-800 dark:bg-gray-600 dark:text-gray-300'}`}>{person.isActive ? 'Active' : 'Inactive'}</span></td><td className="px-6 py-4"><div className="flex space-x-2"><button onClick={() => openModal('view', person)} className="p-1 bg-green-100 dark:bg-green-800 text-green-600 dark:text-green-300 rounded hover:bg-green-200 dark:hover:bg-green-700 transition-colors" title="View Details"><Eye className="h-4 w-4" /></button>{!isStudentView && <button onClick={() => openModal('edit', person)} className="p-1 bg-green-100 dark:bg-green-800 text-green-600 dark:text-green-300 rounded hover:bg-green-200 dark:hover:bg-green-700 transition-colors" title="Edit"><Edit className="h-4 w-4" /></button>}<button onClick={() => onStartConversation(person.id)} className="p-1 bg-teal-100 dark:bg-teal-800 text-teal-600 dark:text-teal-300 rounded hover:bg-teal-200 dark:hover:bg-teal-700 transition-colors" title={isStudentView ? 'Request to Connect' : 'Message'}><Mail className="h-4 w-4" /></button>{!isStudentView && <button onClick={() => confirmDelete(person.id)} className="p-1 bg-red-100 dark:bg-red-800 text-red-600 dark:text-red-300 rounded hover:bg-red-200 dark:hover:bg-red-700 transition-colors" title="Delete"><Trash2 className="h-4 w-4" /></button>}</div></td></tr>))}</tbody></table></div>{filteredAlumni.length === 0 && (<div className="text-center py-12"><Users className="h-12 w-12 text-gray-400 mx-auto mb-4" /><p className="text-gray-500 dark:text-gray-400">{isStudentView ? 'No mentors found matching your criteria' : 'No alumni found matching your criteria'}</p></div>)}</div>
      )}
    </div>
  );
};
